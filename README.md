...
由于本项目依赖 PHP、MySQL 以及常驻后端的队列调度守护进程等复杂的环境配置。为方便使用，我们强烈建议使用提供的 **Docker Compose** 方案。
...
> - 整个编排文件会同时启动 5 个容器微服务（主程序 Web、MySQL 数据库、Mailpit邮件陷阱、队列处理器 Queue Worker 以及定时任务处理进程 Scheduler）
...
# 💣 【危险】停止服务并彻底摧毁所有数据库持久化卷数据（文件数据亦会丢失）
